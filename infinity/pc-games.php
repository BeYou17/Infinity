<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>INFINITY | PC-Games</title>
    <link rel="shortcut icon" href="./Images/INFINITY-LoGo.png">
    <link rel="stylesheet" href="./styles.css">
    <link rel="stylesheet" href="./pc-games.css">
    <link rel="stylesheet" href="path/to/font-awesome/css/font-awesome.min.css">
    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@100;200;300;400;500;600;700;800;900&display=swap"
        rel="stylesheet">
</head>

<body>
    <header>
        <nav class="nav-bar">
            <div class="logo">
                <a href="./content-page.php">
                    <img src="./Images/INFINITY-Logo-White.png" alt="INFINITY-LoGo">
                    <p>INFINITY</p>
                </a>
            </div>
            <div class="nav-links close">
                <ul>
                    <li><a href="./feedb./pc-reg.htmlack-2.html">Contact</a></li>
                    <li><a href="./profile.php">My Profile</a></li>
                    <li><a href="./login.php">Logout</a></li>
                </ul>
            </div>
            <div class="nav-icon">
                <ion-icon name="menu"></ion-icon>
            </div>
        </nav>
    </header>
    <section>
        <div class="container">
            <div class="box">
                <a href="./fortnite.php">
                    <img src="./Images/Fortnite.png" alt="Fortnite">
                    <h2 class='title'>FORTNITE</h2>
                </a>
            </div>
            <div class="box">
                <a href="./csgo.php">
                    <img src="./Images/CS-Go.png" alt="CS-Go">
                    <h2 class='title'>CS GO</h2>
                </a>
            </div>
            <div class="box">
                <a href="./dota.php">
                    <img src="./Images/Dota-2.png" alt="Dota-2">
                    <h2 class='title'>DOTA</h2>
                </a>
            </div>
            <div class="box">
                <a href="./apex.php">
                    <img src="./Images/Apex-Legends.png" alt="Apex-Legends">
                    <h2 class='title'>APEX LEGENDS</h2>
                </a>
            </div>
            <div class="box">
                <a href="./pubgpc.php">
                    <img src="./Images/PUBG-PC.png" alt="PUBG">
                    <h2 class='title'>PUBG PC</h2>
                </a>
            </div>
        </div>
    </section>
    <footer>
        <div class="social-links">
            <a href="" class="twitter link">
                <ion-icon name="logo-twitter"></ion-icon>
            </a>
            <a href="" class="facebook link">
                <ion-icon name="logo-facebook"></ion-icon>
            </a>
            <a href="" class="instagram link">
                <ion-icon name="logo-instagram"></ion-icon>
            </a>
        </div>
        <div class="copyrights">&copy All rights reserved by INFINITY 2020.</div>
        <div class="contacts">
            <p>Email: infinity@gmail.com</p>
            <p>Phone: +91 000000000</p>
        </div>
    </footer>
    <script src="https://unpkg.com/ionicons@5.2.3/dist/ionicons.js"></script>
    <script src="./cj.js"></script>
</body>

</html>